...
def get_current_utc_iso() -> str:
    ...
    # Some validators prefer this format
...
def build_certificate_payload(
    ...
    Args:
        lead: Dictionary containing lead form data with keys:
            ...
            - State, Has Medicare, Ongoing Conditions
    ...
    website = os.getenv("EPERKINS_WEBSITE", "myrpmcare.com")
    source_system = os.getenv("EPERKINS_SOURCE_SYSTEM", "rpm-video-automation")
    ...
    # Add all fields from lead data
    if lead.get("State"):
        form_answers["state"] = str(lead.get("State")).strip()

    if lead.get("Has Medicare"):
        form_answers["has_medicare"] = str(lead.get("Has Medicare")).strip()

    if lead.get("Ongoing Conditions"):
        form_answers["ongoing_conditions"] = str(lead.get("Ongoing Conditions")).strip()

    if lead.get("Contact Consent"):
        form_answers["contact_consent"] = str(lead.get("Contact Consent")).strip()

    if lead.get("Privacy Terms"):
        form_answers["privacy_terms"] = str(lead.get("Privacy Terms")).strip()

    if lead.get("Tax Debt Consent"):
        form_answers["tax_debt_consent"] = str(lead.get("Tax Debt Consent")).strip()
