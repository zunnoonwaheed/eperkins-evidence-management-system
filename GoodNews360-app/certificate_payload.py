"""
Certificate Payload Builder for TheGoodNews360
...
def build_certificate_payload(
    ...
    Args:
        lead: Dictionary containing lead form data with keys:
            ...
            - Age Range, Home Status, Household Income
            - Owe Back Taxes, Monthly Bill Reduction
    ...
    company_key = os.getenv("EPERKINS_COMPANY_KEY", "thegoodnews360")
    website = os.getenv("EPERKINS_WEBSITE", "thegoodnews360.com")
    source_system = os.getenv("EPERKINS_SOURCE_SYSTEM", "goodnews360-video-automation")
    ...
    # Add TheGoodNews360-specific fields
    if lead.get("ZIP Code"):
        form_answers["zip_code"] = str(lead.get("ZIP Code")).strip()

    if lead.get("Age Range"):
        form_answers["age_range"] = str(lead.get("Age Range")).strip()

    if lead.get("Home Status"):
        form_answers["home_status"] = str(lead.get("Home Status")).strip()

    if lead.get("Household Income"):
        form_answers["household_income"] = str(lead.get("Household Income")).strip()

    # Support both "Owe Back Taxes" and "Back Taxes" column names
    back_taxes = lead.get("Owe Back Taxes") or lead.get("Back Taxes") or lead.get("Tax Debt Amount")
    if back_taxes:
        form_answers["owe_back_taxes"] = str(back_taxes).strip()

    # Support both "Monthly Bill Reduction" and "Monthly Bills" column names
    monthly_bills = lead.get("Monthly Bill Reduction") or lead.get("Monthly Bills")
    if monthly_bills:
        form_answers["monthly_bill_reduction"] = str(monthly_bills).strip()
